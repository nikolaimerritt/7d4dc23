...
public class GuessController : AuthenticatedController
{
    ...
    public async Task<IActionResult> Index()
    {
        ...
        var guesses = await _context
            .Guesses.Where(g => g.UserId == int.Parse(userId))
            .Include(g => g.Mastermind)
            .ToListAsync();

        List<Suspect> guessedMasterminds = guesses.Select(g => g.Mastermind).ToList();
        var remainingSuspects = await _context
            .Suspects.Where(s => !guessedMasterminds.Contains(s))
            .ToListAsync();
        ViewBag.Options = remainingSuspects
            .Select(s => new SelectListItem { Value = s.SuspectId.ToString(), Text = s.Name })
            .ToList();

        var flags = _context
            .Flags.OrderByDescending(f => f.Points)
    ...
    {
        Suspect guessedMastermind = _context.Suspects.FirstOrDefault(s =>
            s.SuspectId == mastermindId
        );
        ...
        var doNotStoreGuess = _context.Guesses.Any(g =>
            g.UserId == userId && (g.Mastermind == guessedMastermind || g.Correct)
        );
        if (!doNotStoreGuess)
        {
